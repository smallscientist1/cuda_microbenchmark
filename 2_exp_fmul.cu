...
int main(){
    ...
    cudaMalloc(&d_b, grid.x * block.x * REG_NUM * sizeof(float));
    float ms = do_bench([&]() { k<<<grid, block>>>(d_a, d_b, loops); });
    printf("done in %f ms\nFMA flops: %f TFLOPs/s\nXU flops: %f TFLOPs/s\n", ms, 3*grid.x*block.x*REG_NUM*float(loops/1e9)/ms, 1*grid.x*block.x*REG_NUM*float(loops/1e9)/ms);
