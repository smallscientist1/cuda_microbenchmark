...
__global__ void k(volatile T *__restrict__ d1, volatile T *__restrict__ d2,
  ...
  T s[THREAD_ATOM * 8]; // for 4MB: 1024*1024/512/256 = 8
    ...
    for (int j = (threadIdx.x + blockDim.x * blockIdx.x)*THREAD_ATOM; j < ds;
      ...
      // if (i & 1){
        ...
        for(int k=0;k<THREAD_ATOM;k++)
          // load only
          s[(ii++)*THREAD_ATOM+k] = d2[j+k];
          // store only
          // d1[j+k] = s[(ii++)*THREAD_ATOM+k];
      // }
      // else{
      //   #pragma unroll
      //   for(int k=0;k<THREAD_ATOM;k++)
...
int main() {
  ...
  int csize_list[] = {1048576 * 4 / sizeof(T), 1048576 * 2 / sizeof(T),
                      1048576 * 4 / sizeof(T), 1048576 * 4 / sizeof(T)};
  ...
  for (int i = 0; i < 4; i++) {
    ...
    printf("BW: %f GB/s\n",1.0 * csize * sizeof(T) * iter / ms / 1e6);
  }
}
